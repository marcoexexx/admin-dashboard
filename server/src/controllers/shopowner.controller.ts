...
import { CreateShopownerInput, DeleteMultiShopownersInput, GetShopownerInput, UpdateShopownerInput } from "../schemas/shopowner.schema";
...
export async function getShopownersHandler(
  ...
  try {
    ...
    const { _count, users, exchanges } = convertStringToBoolean(query.include) ?? {}
...
export async function getShopownerHandler(
  ...
  try {
    ...
    // Create audit log
    if (shopowner && sessionUser) (await service.audit(sessionUser)).ok_or_throw()
...
export async function createMultiShopownersHandler(
  ...
  try {
    ...
    const shopowners = (await service.tryExcelUpload(excelFile)).ok_or_throw()
...
export async function deleteShopownerHandler(
  ...
  try {
    ...
    const shopowner = (await service.tryDelete({ where: { id: shopownerId } })).ok_or_throw()
...
export async function updateShopownerHandler(
  req: Request<UpdateShopownerInput["params"], {}, UpdateShopownerInput["body"]>,
  res: Response,
  next: NextFunction
) {
  try {
