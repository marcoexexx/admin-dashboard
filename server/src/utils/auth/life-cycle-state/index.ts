...
import { handleTryOnDraftProductStatus, handleTryOnPendingProductStatus, handleTryOnPublishedProductStatus } from "./productLifeCycleState"
import { handleTryOnCancelledOrderStatus, handleTryOnDeliveredOrderStatus, handleTryOnPendingOrderStatus, handleTryOnProcessOrderStatus, handleTryOnShippedOrderStatus } from "./orderLifeCycleState"
...
if (require.main === module) {
  const productLifeCycleState = new LifeCycleState<LifeCycleProductConcrate>({ resource: "product", state: ProductStatus.Draft })
  let productState = (() => { try {return productLifeCycleState.changeState(ProductStatus.Pending)} catch (err: any) {console.log(err.message)} })()

  const orderLifeCycleState = new LifeCycleState<LifeCycleOrderConcrate>({ resource: "order", state: OrderStatus.Pending })
  let orderState = (() => { try {return orderLifeCycleState.changeState(OrderStatus.Shipped)} catch (err: any) {console.log(err.message)} })()
