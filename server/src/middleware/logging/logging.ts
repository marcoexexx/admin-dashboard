...
const logging: Logging = {
  log: (message, ...opt) => 4 <= level.indexOf(currentLevel) 
    ? console.log(LoggingConfig.display(message, level[4]), ...opt) 
    : undefined,

  info: (message, ...opt) => 3 <= level.indexOf(currentLevel) 
    ? console.info(LoggingConfig.display(message, level[3]), ...opt) 
    : undefined,

  debug: (message, ...opt) => 2 <= level.indexOf(currentLevel) 
    ? console.debug(LoggingConfig.display(message, level[2]), ...opt) 
    : undefined,

  warn: (message, ...opt) => 1 <= level.indexOf(currentLevel) 
    ? console.warn(LoggingConfig.display(message, level[1]), ...opt) 
    : undefined,

  error: (message, ...opt) => 0 <= level.indexOf(currentLevel) 
