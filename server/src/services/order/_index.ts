 ...
 */
export class OrderService implements AppService {
  ...
  async deleteMany<
    T extends { sessionUser: User, filter: Prisma.OrderWhereInput }
  >(args: T): Promise<Result<[Prisma.BatchPayload, Prisma.BatchPayload, Prisma.BatchPayload], AppError>> {
    const try_data = await db
      .$transaction([
        db.orderItem.deleteMany({
          where: {
