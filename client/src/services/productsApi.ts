...
import { GenericResponse, HttpListResponse, HttpResponse, Pagination, Product, ProductSalesCategoriesResponse, QueryOptionArgs } from "./types";
...
export class ProductApiService extends BaseApiService<ProductWhereInput, Product> {
  ...
  async findManySaleCategories(opt: QueryOptionArgs, { productId }: { productId: string | undefined }): Promise<HttpListResponse<ProductSalesCategoriesResponse> | undefined> {
  ...
  async createSaleCategory({ productId, salesCategoryId, discount }: { productId: string, salesCategoryId: string, discount: number }): Promise<GenericResponse<ProductSalesCategoriesResponse, "category"> | undefined> {
    const url = `/${this.repo}/detail/${productId}/sales`
  ...
  async deleteSaleCategory({ productId, productSaleCategoryId }: { productId: string, productSaleCategoryId: string }): Promise<HttpResponse | undefined> {
  ...
  async updateSaleCategory({ productId, productSaleCategoryId, discount }: { productId: string, productSaleCategoryId: string, discount: number }): Promise<GenericResponse<ProductSalesCategoriesResponse, "productSalesCategory"> | undefined> {
    const url = `/${this.repo}/detail/${productId}/sales/detail/${productSaleCategoryId}`
  ...
  async like({ userId, productId }: { userId: string, productId: string }): Promise<GenericResponse<Product, "product"> | undefined> {
