...
const filterRolesSchema = object({
  name: string().min(0).max(128).optional(),
  insensitive: boolean().optional().default(false),
})

export type FilterRolesInput = z.infer<typeof filterRolesSchema>

export function RolesFilterForm() {
  const { dispatch } = useStore()

  const [filterQuery, setFilterQuery] = useSearchParams()

  const methods = useForm<FilterRolesInput>({
    resolver: zodResolver(filterRolesSchema)
  })

  const { handleSubmit, register, formState: { errors }, setValue } = methods

  const onSubmit: SubmitHandler<FilterRolesInput> = (value) => {
    const { name, insensitive } = value

    setFilterQuery(prev => ({ ...prev, ...value }))

    dispatch({
      type: "SET_ROLE_FILTER", payload: {
        where: {
          name: {
