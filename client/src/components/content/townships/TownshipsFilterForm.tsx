...
const filterTownshipsSchema = object({
  name: string().min(0).max(128).optional(),
  insensitive: boolean().optional().default(false),
})

export type FilterTownshipsInput = z.infer<typeof filterTownshipsSchema>

export function TownshipsFilterForm() {
  const { dispatch } = useStore()

  const [filterQuery, setFilterQuery] = useSearchParams()

  const methods = useForm<FilterTownshipsInput>({
    resolver: zodResolver(filterTownshipsSchema)
  })

  const { handleSubmit, register, formState: { errors }, setValue } = methods

  const onSubmit: SubmitHandler<FilterTownshipsInput> = (value) => {
    const { name, insensitive } = value

    setFilterQuery(prev => ({ ...prev, ...value }))

    dispatch({ type: "SET_TOWNSHIP_FILTER", payload: {
      fields: {
        name: {
