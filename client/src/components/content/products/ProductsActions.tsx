...
interface ProductsActionsProps {
  onExport: () => void
  onImport: (data: CreateProductInput[]) => void
  isAllowedImport: boolean
}

export function ProductsActions(props: ProductsActionsProps) {
  ...
  const handleChangeImportExcel = (evt: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      ...
      const parsedData = XLSX.utils.sheet_to_json(sheet) as CreateProductInput[]
