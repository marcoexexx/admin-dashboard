...
interface CategoriesActionsProps {
  onExport: () => void
  onImport: (data: CreateCategoryInput[]) => void
  isAllowedImport: boolean
}

export function CategoriesActions(props: CategoriesActionsProps) {
  ...
  const handleChangeImportExcel = (evt: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      ...
      const parsedData = XLSX.utils.sheet_to_json(sheet) as CreateCategoryInput[]
